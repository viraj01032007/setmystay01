...
export function ListingsSection({ type, listings, onViewDetails, onSmartSort }: ListingsSectionProps) {
  ...
  const filteredListings = useMemo(() => {
    return listings.filter(item => {
      if ('propertyType' in item) { // It's a Listing
    ...
    <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div className="flex flex-col md:flex-row gap-8">
        <aside className="md:w-1/3 lg:w-1/4">
          <div className="p-6 bg-card rounded-xl shadow-sm sticky top-24">
            ...
            {(type === 'rental' || type === 'pg') && (
              <div className="space-y-4">
                ...
                <div className="space-y-2">
                    ...
                    <RadioGroup value={filters.furnishedStatus} onValueChange={(val) => handleFilterChange('furnishedStatus', val)} className="flex gap-2">
